    ...
    */
    ...
    public static class Person {
        ...
        void takeDamage(Integer damage) throws Exception {
            ...
            if((health - (damage - protection))>0) {
                this.health = health - (damage - protection);
            }
            else {
                this.health=0;
    ...
    public static class Mage extends Person{
        ...
        @Override
        String announce(){
            return "Mage "+getName()+" имеет характеристики: "+getHealth()+" здоровья и "+getProtection()+" защиты";
        }
        @Override
        void takeDamage(Integer damage) throws Exception {
            if(damage<0) throw new Exception();
            if(getHealth() - (damage - getProtection()-getHealth()%10)>0)
...
//            takeDamage(45);
    ...
    public static class Archer extends Person{
        ...
        @Override
        String announce(){
            return "Archer "+getName()+" имеет характеристики: "+getHealth()+" здоровья и "+getProtection()+" защиты";
        }
        @Override
        void takeDamage(Integer damage) throws Exception {
            if(damage-getProtection()<0) throw new Exception();
            if(getHealth() - (damage - getProtection()+getHealth()%10)>0)
                super.takeDamage(damage+getHealth()%10);
            else throw new PersonDead("Person dead.");
        }
        int shootBow() throws Exception{
